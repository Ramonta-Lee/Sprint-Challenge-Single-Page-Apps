...
> Answer:
> The benefits of using client-side routing is that it offers control over your site URL (address bar) and content without necessarily having to wait for the server to respond with content. It is designed to mimic the appearance of traditional multi page websites. In client-side routing JavaScript handles the routing internally and now we don't have to get an entire page refresh like we do when we request information from a server and the whole page is re-rendered with the request or completed action. JavaScript gets the resource that was already available to us and renders out the information. Basically when a user clicks on a link the URL changes but the request to the server is prevented by JavaScript. The adjustment to the URL results in a changed state of the application. This change in state will ultimately result in a different view of the webpage but the page won't refresh when using client-side routing just some elements inside the application will change. Less data is processed in this way, routing between views is generally faster. The user has smooth transitions and animations between views are easier to implement.

Some downsides to keep in mind is that the whole website or web-application needs to be loaded on the first request which could initially make loading the page take longer. Also because the whole page is loaded initially, there is a chance that there is data downloaded for views you won't even need. It requires more setup work or even a library because server-side is the standard, so extra code must be written to make client-side routing possible.
